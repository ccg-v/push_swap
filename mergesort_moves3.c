...
void push_to(node** from_stack, node** to_stack, node** stack_a) {
...
*/
void sort(node** stack1, node** stack2, node** stack_a,  int size) {
    if (size < 2) {
        return;
    }
    int middle = size / 2;
    for (int i = 0; i < middle; i++) {
        push_to(stack1, stack2, stack_a);
    }
    sort(stack2, stack1, stack_a, middle);
    sort(stack2, stack1, stack_a, size - middle);
    while (*stack1 != NULL && *stack2 != NULL) {
        if ((*stack1)->value < (*stack2)->value) {
            push_to(stack1, stack2, stack_a);
        } else {
            push_to(stack2, stack1, stack_a);
        }
    }
    while (*stack1 != NULL) {
        push_to(stack2, stack1, stack_a);
...
int main() {
	...
	sort(&stack_a, &stack_b, &stack_a, MAX_SIZE);
